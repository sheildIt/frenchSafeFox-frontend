  ...
  return (
    <div className='flex flex-col p-5'>
      ...
      <div className='flex flex-row p-5'>
        <div className='bg-white w-96 h-[550px] rounded-2xl flex flex-col p-5 relative'>
          <p className='text-xl text-black font-normal p-2'>Send configuration</p>
          <div className='relative'>
            <select onChange={handleSelectedDepartment} className='w-full rounded-md h-10 mt-5 bg-creme text-grayLight font-normal pl-3 pr-8 cursor-pointer'>
          ...
          <p className='txt-normal text-black font-normal text-justify p-2 mt-5'>Recipients: ({selectedDep?.number_of_employees ?? 0})</p>
          <div className='flex flex-row h-32 p-2'>
            
            <ul className='inline-flex text-black'>{employees?.map((employee_obj)=>{
              return <EmployeeCard key={employee_obj.id} employee={employee_obj}>{employee_obj}</EmployeeCard>
            })}</ul>
            
          </div>
          <div className='flex flex-col p-2 bg-creme rounded-md h-[150px] mt-2'>
          <div className='flex flex-row p-2'>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-grayLight">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z" />
              </svg>
             <div className='flex flex-col ml-2'>
             <p className='text-grayLight text-justify font-normal'>Date</p>
             <p className='text-grayLight text-justify font-normal'>12/02/2024</p>
             </div>
          </div>
          <div className='flex flex-row p-2'>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-grayLight">
          ...
          <div className='flex flex-col ml-2'>
             <p className='text-grayLight text-justify font-normal'>Time</p>
             <p className='text-grayLight text-justify font-normal'>8:00 AM</p>
             </div>
          ...
          <button onClick={sendEmail} className='w-20 absolute bottom-4 bg-green-600'>Send</button>
        </div>
      </div>
    </div>
  )
